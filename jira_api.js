...
class JiraApi {
  ...
  async getData(url = "") {
    const basicAuth = Buffer.from(`${this.username}:${this.token}`).toString(
      "base64"
    );
  ...
  async getProjectFromJira(projectKey) {
    const data = await this.getData(
      `https://${this.baseUrl}/rest/api/3/project/${projectKey}`
    );
  ...
  async createVersion(projectKey, version, releaseDateIsoString) {
    ...
    try {
      const data = await this.postData(
        `https://${this.baseUrl}/rest/api/3/version`,
        {
          archived: false,
      ...
      console.log(
        `https://${this.baseUrl}/projects/${projectKey}?selectedItem=com.atlassian.jira.jira-projects-plugin%3Arelease-page`
      );
  ...
  async addIssueToVersion(issueKey, version) {
    try {
      const response = await this.putData(
        `https://${this.baseUrl}/rest/api/3/issue/${issueKey}`,
        {
          update: {
            fixVersions: [
              {
  ...
  async tagIssueWithEnvironmentAction(issueKey, action, environment) {
    await this.addLabelToIssue(issueKey, `${action}-${environment}`);
  }

  async addLabelToIssue(issueKey, label) {
    try {
      const response = await this.putData(
        `https://${this.baseUrl}/rest/api/3/issue/${issueKey}`,
        {
      ...
      console.log(`Added Label "${label}" to Issue: ${issueKey}`);
      return response;
    } catch (e) {
      console.log(e);
    }
