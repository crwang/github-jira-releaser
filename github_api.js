...
class GithubApi {
  ...
  async compareBetweenCommits(commit1, commit2) {
    const data = await this.getData(
      `https://${API_HOST}/repos/${this.owner}/${this.repo}/compare/${commit1}...${commit2}`
    );
    return data.commits;
  }
  ...
  async compareBetweenTags(tag1, tag2) {
    const data = await this.getData(
      `https://${API_HOST}/repos/${this.owner}/${this.repo}/compare/${tag1}...${tag2}`
    );
    return data.commits;
  }
  ...
  async getPreviousDateGitTagObject(currDateTag, firstPatch = false) {
    ...
    const prevTag = data.data.repository.refs.nodes.find((ref) => {
      if (firstPatch) {
        return (
          versioning.isFirstPatch(ref.name) &&
          versioning.versionMoreRecentThan(currDateTag, ref.name)
        );
      } else {
